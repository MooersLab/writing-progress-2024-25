...
# Generate a date range. 2024 is a leap year with 366 days.
all_days = pd.date_range('1/1/2024', periods=731, freq='D')

# Fetch the daily minute total.
L = sh.worksheet("min_day").get('E2:E732')
...
minutes = pd.Series(int_list, index=all_days)
#print(minutes.to_string())

calplot.calplot(minutes, cmap='Blues', figsize=(16,5.5));
plt.suptitle('Manuscript Writing Effort in Minutes per Day', y=1.0, fontsize=20);
plt.show()
